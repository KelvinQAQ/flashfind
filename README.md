...
结果列表采用 `d/f | 文件名 | 大小 | 修改时间` 的响应式列布局：`d` 为目录，`f` 为其他文件条目；文件名不展示父路径，大小自动使用 `B/KiB/MiB/GiB/TiB`，修改时间为本地时区的 `YYYY-MM-DD hh:mm:ss`。终端变窄时先隐藏修改时间，再隐藏大小；名称始终保留，并在必要时以中间 `…` 省略、保留扩展名或名称尾部。
